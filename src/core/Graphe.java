...
public class Graphe {
	...
	// Tableau qui va contenir tous les sommets et donc tous les successeurs de
	// chaque sommet
			...
			for (int num_node = 0; num_node < nb_nodes; num_node++) {
				for (int num_succ = 0; num_succ < nsuccesseurs_a_lire[num_node]; num_succ++) {
					...
					for (int i = 0; i < nb_segm; i++) {
						...
						dessin.drawLine(current_long, current_lat, (current_long + delta_lon),
								(current_lat + delta_lat));
						current_long += delta_lon;
						current_lat += delta_lat;
					}
					// Si double sens on rajoute l'arc à la liste des
					// successeurs du successeur du noeud courant
					if (descripteurs[descr_num].isSensUnique() == false) {
						 ...
						 * tabNoeud[dest_node].ajouterSegment(tabNoeud[dest_node
						 * ].numDernierElement(),
						 * a1.recupSegment(i,2),a1.recupSegment(i,3),a1.
						 * recupSegment(i,0),a1.recupSegment(i,1)); }
						 */
					}
					...
					if (succ_zone == numzone) {
						dessin.drawLine(current_long, current_lat, tabNoeud[dest_node].getLongitude(),
								tabNoeud[dest_node].getLatitude());
						// on ajoute les segments droits entre 2 noeuds
						tabNoeud[num_node].ajouterSegment(tabNoeud[num_node].numDernierElement(), current_long,
								current_lat, tabNoeud[dest_node].getLongitude(), tabNoeud[dest_node].getLatitude());
						a.ajouterSegment(tabNoeud[num_node].getLongitude(), tabNoeud[num_node].getLatitude(),
								current_long, current_lat);
					}
				}
			}
			Utils.checkByte(253, dis);
			System.out.println("Fichier lu : " + nb_nodes + " sommets, " + edges + " aretes, " + nb_descripteurs
					+ " descripteurs.");
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(1);
	...
	public void verifierChemin(DataInputStream dis, String nom_chemin) {
		try {
			...
			if (path_carte != this.idcarte) {
				System.out.println(
						"Le chemin du fichier " + nom_chemin + " n'appartient pas a la carte actuellement chargee.");
				System.exit(1);
			}
			...
			for (int i = 0; i < nb_noeuds; i++) {
				...
				leChemin.FormerListeSuccesseur(current_node); // code perso , on
																// forme la
																// liste des
																// sommets du
																// chemin
	...
	public float calculerDistance() {
		...
		for (int i = 0; i < leChemin.getNombreSommet() - 1; i++) {
			distance_chemin += tabNoeud[leChemin.accederElement(i)].recupererlongueur(leChemin.accederElement(i),
					leChemin.accederElement(i + 1));
		}
	...
	public float calculertemps() {
		...
		int numerateur;
		int denominateur;
		ArrayList<Segment> tabDeSegment = new ArrayList<Segment>(); // tableau
																	// de
																	// segment
																	// pour
																	// dessiner
		...
		for (int i = 0; i < leChemin.getNombreSommet() - 1; i++) {
			...
			float temps_courant = tabNoeud[leChemin.accederElement(i)].temps_min(leChemin.accederElement(i),
					leChemin.accederElement(i + 1));
			// on déssine un point sur le sommet courant
			dessin.setColor(java.awt.Color.blue);
			dessin.drawPoint(tabNoeud[leChemin.accederElement(i)].getLongitude(),
					tabNoeud[leChemin.accederElement(i)].getLatitude(), 5);
		}
		// on dession un point sur le dernier sommet
		dessin.drawPoint(tabNoeud[leChemin.accederElement(leChemin.getNombreSommet() - 1)].getLongitude(),
				tabNoeud[leChemin.accederElement(leChemin.getNombreSommet() - 1)].getLatitude(), 5);
		return temps_chemin;
	}

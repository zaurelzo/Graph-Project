...
public class BinaryHeap<E extends Comparable<E>> {
	...
	public ArrayList<E> array; // The heap array
	...
	private BinaryHeap(BinaryHeap<E> heap) {
		...
		this.H = new HashMap<E, Integer>(heap.H);
	...
	private void arraySet(int index, E value) {
		if (index == this.array.size()) {
			...
			this.H.put(value, index); // om map la valeur
		} else {
			this.array.set(index, value);
			this.H.put(value, index);
		}
	}
	...
	public void update(E x) {
		int indice = this.H.get(x);
		percolateUp(indice);
		// percolateDown(indice);
	}

	public int exist(E x) {
	...
	/*
	 * public static void main(String [] args) { BinaryHeap<Integer> heap = new
	 * BinaryHeap<Integer>() ; PriorityQueue<Integer> queue = new
	 * PriorityQueue<Integer>() ;
	 * 
	 * int count = 0 ; int blocksize = 10000 ;
	 * 
	 * System.out.println("Interrupt to stop the test.") ;
	 * 
	 * while (true) {
	 * 
	 * // Insert up to blocksize elements int nb_insert = (int)(Math.random() *
	 * (blocksize + 1)) ;
	 * 
	 * for (int i = 0 ; i < nb_insert ; i++) { Integer obj = new Integer(i) ;
	 * heap.insert(obj) ; queue.add(obj) ; }
	 * 
	 * // Remove up to blocksize elements int nb_remove = (int)(Math.random() *
	 * blocksize * 1.1) ;
	 * 
	 * if (nb_remove > queue.size()) { nb_remove = queue.size() ; }
	 * 
	 * for (int i = 0 ; i < nb_remove ; i++) {
	 * 
	 * int removed1 = queue.poll().intValue() ; int removed2 =
	 * heap.deleteMin().intValue() ;
	 * 
	 * if (removed1 != removed2) { System.out.println("Ouch : expected " +
	 * removed1 + "  .. but got " + removed2) ; System.exit(1) ; } }
	 * 
	 * if (heap.size() != queue.size()) {
	 * System.out.println("Ouch : heap size = " + heap.size() +
	 * "  queue size = " + queue.size() ) ; System.exit(1) ; }
	 * 
	 * count += nb_remove ;
	 * 
	 * if (count > 1000000) { System.out.println("" + count +
	 * " items successfully compared. Heap size : " + heap.size()) ; count = 0 ;
	 * } } }
	 */
}

...
public class Noeud {
	...
	public void ajouterSegment(int num_arc, float debut_longitude, float debut_latitude, float fin_longitude,
			float fin_latitude) {
	...
	// recupere la vitesse minimale entre un noeud source et un noeud de
	// destination
	public int recupererVitesse(int NumSommetSource, int NumSOmmetDestination) {
		...
		for (int i = 0; i < arcSortants.size(); i++) {
			if (arcSortants.get(i).getNumSource() == NumSommetSource
					&& arcSortants.get(i).getNumDest() == NumSOmmetDestination) {
	...
	// recupere la logueur minimale entre un noeud source et un noeud de
	// destination
	...
	public ArrayList<Segment> recupererLeSegment(int sommentSource, int SommetDestination) {
		// se base sur la recherche de la vitesse max pour rechercher les bons
		// segments à dessiner ;
		...
		int vitmax = 0;
		for (int i = 0; i < arcSortants.size(); i++) {
			if (arcSortants.get(i).getNumSource() == sommentSource
					&& arcSortants.get(i).getNumDest() == SommetDestination) {
		...
		// debug
		if (lesBonSegments.size() == 0) {
			System.out.println("pas de segment entre : " + sommentSource + " et " + SommetDestination);
			System.exit(1);
		}
